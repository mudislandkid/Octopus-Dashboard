...
function validateApiEndpoint(endpoint: string | undefined): string {
  if (!endpoint) {
    return 'https://api.octopus.energy/v1'
  }
...
export const config: EnvConfig = {
  OCTOPUS_API_KEY: validateApiKey(process.env.VITE_OCTOPUS_API_KEY),
  OCTOPUS_API_ENDPOINT: validateApiEndpoint(process.env.VITE_OCTOPUS_API_ENDPOINT),
} 